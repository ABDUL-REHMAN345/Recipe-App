...
// Searching API Functionality jo cheez search ho ge woi ahey ge:
queryStr.split("&").forEach(i => {
  const [key, value] = i.split("=");

  if (key === "q") {
    if (value) {
      $filterBar.querySelector("input[type='search']").value = decodeURIComponent(value.replace(/\+/g, ' '));
    }
  } else {
    if (value) {
      const element = $filterBar.querySelector(`[value="${decodeURIComponent(value.replace(/\+/g, ' '))}"]`);
      if (element) {
...
// Unlimited data Fetch from API function
